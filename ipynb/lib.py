...
def get_var_process(searching_key):
...
def start_task(process, body=''):
    request = http()
    request.url = '' + get_values_config('CONEXAO', 'bpmProtocol') + '://' + get_values_config('CONEXAO',
                                                                                           'ipBpmProd') + ':' + get_values_config(
        'CONEXAO', 'envBpmPort') + '/engine-rest/process-definition/key/' + process + '/start'
    request.set_header('Authorization', 'Basic YnBhOmJwNEA0c3Mxc3Q=')
    request.set_header('Content-Type', 'application/json')

    if (body == ''):
        request.body = convert_json_bpm('brain', get_values_config('CONTROLE', 'nomeProcesso'))
    else:
        request.body = body
...
def start_multi_producer():
    ...
    return start_task(get_values_config('CONTROLE', 'multiProducerTo'),
                      convert_json_bpm('multiProducer', get_values_config('CONEXAO', 'multiProducerTo')))
...
#
# >> 2020/03/28 04:41:10 - delete_file_sure(file_path='C:\prd-SegundaViaBoleto\Processando-RoboSegundaViaBoleto\process_info_memory', safe_extension_check='', timeout=5):
...
############################################################################
def get_passed_parameters(func_name, local_dict):
    '''
    Inputs
...
try:
    race_mode = config['CONTROLE']['raceMode']
except KeyError:
    race_mode = "training"
