...
def ultradebug(string=None, title="UltraDebug", timeout=2):
    ...
    # if [CONTROLE]showDebug=true
    if get_values_config(session="CONTROLE", key="showDebug", warn_on_keyerror=False):
...
def start_task(process, payload=None):
    ...
    if proxy_url == "ERROR":
        ...
        # add ports to proxy_url if doesnt have already
        proxy_url = f"{proxy_url}:3128" if not ":" in proxy_url else proxy_url

        proxy_dict = {
          'http': f'http://{http_credentials_str}{proxy_url}',
          'https': f'https://{http_credentials_str}{proxy_url}',
        }
